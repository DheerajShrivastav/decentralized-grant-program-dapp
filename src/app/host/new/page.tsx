...
const CreateNewEvent = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    status: '',
    winnerProposal: null,
    timeline: [] as { date: string; description: string }[], // Initialize timeline as an array
  })
  ...
  const handleTimelineAdd = () => {
    if (timelineDate && timelineDescription) {
      setFormData({
        ...formData,
        timeline: [
          ...formData.timeline,
          { date: timelineDate, description: timelineDescription },
        ],
      })
      setTimelineDate('')
  ...
  const handleSubmit = async (e: { preventDefault: () => void }) => {
    ...
    const dataToSubmit = {
      ...formData,
      prizes: formData.prizes.filter((prize) => prize.title || prize.amount),
      banner: formData.banner ? await uploadImage(formData.banner) : null,
    }
          ...
          <div className="bg-orange-50 p-6 rounded-lg">
            ...
            <div className="mt-4">
              {formData.timeline.map((entry, index) => (
                <div
                  key={index}
                  className="flex justify-between bg-gray-100 p-2 rounded-md mb-2"
                >
                  <div>
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      const updatedTimeline = formData.timeline.filter(
                        (_, i) => i !== index
                      )
